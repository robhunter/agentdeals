...
export const NAMED_SUBSET_FIELD_RULE =
  "No field picks a few members out of a tied set, because being tied is the absence of one. verifiedDate is the nearest candidate, and every date we hold is like it — the day we last confirmed a record's terms, or the day we last read its page — so each of them records what our own re-read queue has done rather than anything about the vendor, and ordering a list by one publishes that queue as though it were a recommendation. It would also still move — that queue confirms about a hundred records a day, so the three names would be different ones next week for a reason no reader of the claim can see. A record's name, slug, id and position in our file are inputs the tie-break promises never to use, and we hold no date for when we first indexed one.";

                ...
                properties: {
                  ...
                  superseded_terms: {
                    ...
                    properties: {
                      ...
                      offers_we_refuse_to_restate: { type: "object", additionalProperties: { type: "integer" }, description: "The rest, counted by the ground we refuse on. A reading that prices a different plan than the entry describes is not a restatement of that entry. Nor is one that reports on the page it was read from rather than stating that page's terms, one that states no figure where the terms it would replace state several, or one that drops the cap on how many people may use the free tier — a restatement may drop a caveat, but not the fact that decides the tier. One ground is not a rule and says so in its name: it counts the readings held back by name, each of which is waiting on a rule that should reach it. One counts entries whose restatement was put back by hand: the record it was taken from may not restate it a second time, though a newer record contradicting the entry still may." },
                      ...
                      restatements_keeping_the_stored_sentence_saying_what_the_product_is: { type: "integer", description: "How many of the restatements we may make open on the stored clause saying what the product is, with only the terms after it replaced. The rest hold no such clause and take the reading whole." },
                      offers_whose_terms_came_from_a_reading: { type: "integer", description: "Offers whose stored description was taken from a dated reading of the vendor's page rather than written by us or confirmed against that page. This is a fourth state beside confirmed, stamped and read — a read disagreed with us and we took its words — and it is counted here and nowhere else, because neither verifiedDate nor the confirmation date may move on a restatement." },
                      offers_restated_by_change_type: { type: "object", additionalProperties: { type: "integer" }, description: "Those offers counted by the verdict of the change record the reading came from." },
                      oldest_reading_we_publish_as_our_terms: { type: "string", format: "date", nullable: true, description: "The oldest reading standing as an offer's terms. How far back the longest-standing restatement goes." },
                      newest_reading_we_publish_as_our_terms: { type: "string", format: "date", nullable: true, description: "The newest such reading." }
                    }
                  }
                }
              }
    ...
    schemas: {
      ...
      Offer: {
        ...
        properties: {
          ...
          last_read_outcome: { type: "string", nullable: true, description: "What the read on last_read_date concluded, from the verification store's own word for it — confirmed, changed, states_no_price or link_ok, or the source check's own outcome where the store holds no attempt (#1551). Null where neither names that day, which happens when a later attempt failed and last_read_date therefore names an earlier read. Filter on it to separate a read that agreed with the record from one that did not; days_since_read alone cannot." },
          last_read_found: { type: "string", nullable: true, description: "The clause we publish for what that read found, or null where it confirmed the record. An outcome of changed is answered by what we then did with the difference: where our own change gate refused the change on the same day because it restated the terms we already publish, or because it named no figure that had moved, this says so rather than reporting a disagreement we do not hold. Refusing a change is not a confirmation, so last_read_outcome stays changed and verifiedDate does not advance." },
      ...
      SourceCheck: {
        ...
        properties: {
          ...
          detail: { type: "string", description: "Our own sentence recording what the check found — never a quotation from the page, and not corroborable against it (#1467). For ok, the form of the vendor's name the check matched and the figures the page states that this record also publishes — a stated price of zero where none of them does, and a sentence saying we matched none of the amounts we read to the terms this record publishes where the page states amounts and none of them matched (#1871, #1874) — a statement about our own reading, never about what the page contains — or what the markup states when the rendered page states no amount; for states_a_free_price and states_no_amount, the phrase that was the page's entire price evidence; otherwise why the page cannot confirm the record. Where the page was read for schema.org markup, the detail says whether that markup was absent, present and priceless, or priced. The fields carrying text taken from a page are product_role.source_quote and product_subtypes.labels[].source_quote." },

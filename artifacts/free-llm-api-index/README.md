...
We re-read records on a rolling schedule. 0 rows say in the row that we have not re-read them within our 74-day interval. That is a statement about us, not about the vendor. Where a link has stopped resolving, the row says that instead, because we cannot re-read a page that does not answer.
...
| [LLM7.io](https://agentdeals.dev/vendor/llm7-io)<br>Free | Our record, read from [llm7.io](https://llm7.io) on 2026-08-17: UK-based free LLM inference gateway. Free tier supported by donors — access to 30+ models including DeepSeek R1, Qwen2.5 Coder, text, image, and speech-to-text models. No published rate limits on free tier. | `unrated` | We could not read the page we cite for LLM7.io when we last looked, on 2026-09-02. | 2026-08-17 |

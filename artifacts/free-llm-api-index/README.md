...
28 records for vendors that serve models behind an API, each one carrying the date we last read the vendor's own page and the URL we read it on. There is no single freshness stamp for this file, because a single stamp for a list nobody re-read is worth nothing.
...
28 records carry one of those. The catalogue holds them under **AI / ML** and **AI Coding**, and 55 records there are left out. Each is left out for a stated reason, counted here so the size of each reason is visible:
...
| `another_function` | 24 | The record is labelled, and every label it carries names a different function — observability, evaluation, labelling, generation and the rest are not the serving of a model behind an API. |
| `reading_names_no_price_of_nothing` | 3 | The reading this row would publish as current names no price of nothing: no free plan, no zero price, only a price sheet. A tier field saying otherwise is older than that reading, so the reading wins and the row is not published under a title offering free tiers. |
...
The other reasons state what we have or have not read and are counted; this one is a claim about a particular record, which a reader can check against the same page we read, so it names them. Left out under `reading_names_no_price_of_nothing`: **DeepSeek API** (tier `Free Credits + Pay-as-you-go`), **MiniMax** (tier `Pay-as-you-go`), **xAI** (tier `Free Credits`). Each is published at https://agentdeals.dev with the reading that excluded it.
...
| Recorded as ended | 1 |
| Publishing a reason instead of a rating | 6 — `gate:not_a_free_offer` 2, `states_no_terms` 2, `unreadable` 2 |
...
- `unrated` — we are publishing no rating, and the next column says why. 6 of 28 rows are unrated. A record whose page we could not read, that names no terms we can read, that is not a free offer, or whose link has stopped resolving gets the reason instead of a verdict. We would rather print why we cannot say than guess.
...
We re-read records on a rolling schedule. 0 rows say in the row that we have not re-read them within our 72-day interval. That is a statement about us, not about the vendor. Where a link has stopped resolving, the row says that instead, because we cannot re-read a page that does not answer.
...
Anything else is an ongoing free tier. One record is recorded as ended (`an offer the vendor has ended`); they stay in the file because a free tier that has gone is the thing hardest to find out elsewhere.

...
| --- | --- |
| Records | 31 |
| Carrying a rating | 19 |
| Recorded as ended | 2 |
| Publishing a reason instead of a rating | 10 — `gate:not_a_free_offer` 3, `reading_names_no_price_of_nothing` 3, `states_no_terms` 2, `unreadable` 2 |
...
- `unrated` — we are publishing no rating, and the next column says why. 10 of 31 rows are unrated. A record whose page we could not read, that names no terms we can read, that is not a free offer, whose terms name no free price, or whose link has stopped resolving gets the reason instead of a verdict. We would rather print why we cannot say than guess.
...
| [Cohere](https://agentdeals.dev/vendor/cohere)<br>Trial Key | Our record, read from [cohere.com/pricing](https://cohere.com/pricing) on 2026-08-16: AI model API. Trial key: 1,000 API calls/month across all endpoints (Chat, Embed, Rerank). Access to Command R+, Rerank 3.5, Embed 4. Non-commercial use only | `unrated` | The terms this row publishes name no price of nothing — no free plan, no zero price, nothing stated as costing nothing. A tier field saying otherwise is older than the terms beside it. We hold no record of Cohere removing a free tier, so we do not say one ended — we publish no rating and leave the terms to be read. | 2026-08-16 |

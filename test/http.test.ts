...
describe("HTTP transport", () => {
  ...
  it("GET /api/changes includes all_time_total alongside 30-day total", async () => {
    ...
    assert.strictEqual(body.change_census.retrievable_from_this_door, allBody.total, "the retrievable figure should match the since=2000 total");
    assert.strictEqual(body.all_time_total, body.change_census.tracked_pricing_changes, "all_time_total should be the published size of the change log");
  });
  ...
  it("GET /changes renders deal change timeline page", async () => {
    ...
    assert.ok(html.includes("Tracked Changes"), "Should show the tracked-change stat label");
  ...
  it("GET /api/freshness returns freshness metrics", async () => {
    ...
    assert.ok(typeof body.stamped_within_7_days === "number");
    assert.ok(typeof body.stamped_within_30_days === "number");
    assert.ok(typeof body.stamped_within_90_days === "number");
    assert.ok(typeof body.stamped_within_180_days === "number");
    assert.ok(typeof body.confirmed_within_7_days === "number");
    assert.ok(typeof body.confirmed_within_30_days === "number");
    assert.ok(typeof body.confirmed_within_90_days === "number");
    assert.ok(typeof body.confirmed_within_180_days === "number");
    assert.ok(typeof body.stamp_score === "number");
    assert.ok(typeof body.offers_holding_a_confirmation === "number");
  ...
  it("GET /hetzner-pricing-2026 renders Hetzner pricing analysis page", async () => {
    ...
    assert.ok(response.headers.get("content-type")?.includes("text/html"));
    const html = await response.text();
    assert.ok(html.includes("Hetzner Cloud Pricing After the April and June 2026 Adjustments"), "Should have title");
    assert.ok(html.includes("application/ld+json"), "Should have JSON-LD");
    assert.ok(html.includes('"Article"'), "Should use Article schema");
    assert.ok(html.includes("canonical"), "Should have canonical link");
    assert.ok(html.includes("global-nav"), "Should have global nav");
    assert.ok(html.includes("Every Cloud Plan, Priced Today"), "Should have the current pricing table section");
    assert.ok(html.includes("What the April 1 Adjustment Did"), "Should have the April history section");
    assert.ok(html.includes("Why Prices Rose"), "Should have why section");
    assert.ok(html.includes("Who Each Adjustment Applies To"), "Should have who section");

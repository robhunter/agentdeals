...
Commit changes as you progress. Each commit should ideally address only a single task/subtask — make smaller, more easily reviewable commits.
...
**Write the full stack before reviewing.** Create all the stacked PRs first. In reviewed mode (see `operating-modes.md`), review them afterward rather than reviewing each one inline as you go. Subagents can pick up review tasks even if they didn't write the original code.

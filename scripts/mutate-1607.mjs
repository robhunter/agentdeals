...
const MUTANTS = [
  ...
  ["src/serve.ts",
    ...
    "  const heldOpen = false;",
    "a published page drops out when its vendor count falls"],
  ["src/serve.ts",
    "  const heldOpen = bestOfPublishedBefore.has(slug) && generallyAvailable.length > 0;",
    "  const heldOpen = bestOfPublishedBefore.has(slug);",
    "a published page is held open with no record behind it"],
  ["src/serve.ts",
  ...
  ["src/serve.ts",
    ...
    "    excluded.length,",
    "the gate disclosure counts only the offers it gates as held"],
];

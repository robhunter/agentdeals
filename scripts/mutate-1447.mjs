...
const MUTANTS = [
  ...
  ["a-discovered-entry-reads-like-an-event-date", "src/change-feed.ts",
    ...
    "  return true\n    ? `${EFFECTIVE_DATE_PREFIX} ${change.date} · recorded ${recorded}.`"],
  ["the-undated-clause-does-not-say-it-is-the-day-we-recorded", "src/change-feed.ts",
    ...
    "`${DISCOVERED_DATE_PREFIX} ${change.date} · ${UNKNOWN_EFFECTIVE_DATE_MARKER}.`"],
  ...
  ["the-feed-dates-an-entry-in-words-the-page-does-not-use", "src/change-feed.ts",
    "    ? `${EFFECTIVE_DATE_PREFIX} ${change.date} · recorded ${recorded}.`",
    "    ? `in force from ${change.date} · recorded ${recorded}.`"],

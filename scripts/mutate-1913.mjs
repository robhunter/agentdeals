...
const SUITE = "test/eligibility-disclosure.test.ts";

const MUTANTS = [
  ["the-qualification-is-rendered-after-the-vendor-list", "src/serve.ts",
    ...
    "  const catVendorClause = uncontradictedVendorClause(catUncontradicted.map(o => o.vendor), DESCRIPTION_CHARACTER_CAP - catMeasured.length);\n  const metaDesc = catGatedClause?.includes(\"qualification\") && catVendorClause\n    ? `${catMeasured.replace(` ${catGatedClause}`, \"\")}${catVendorClause} ${catGatedClause}`\n    : catMeasured + catVendorClause;",
    ["a category holding one restricted offer and one we name appends the clause after the vendor list"]],

  ["no-description-names-a-vendor", "src/serve.ts",
    "function uncontradictedVendorClause(vendors: readonly string[], room: number): string {\n",
    "function uncontradictedVendorClause(vendors: readonly string[], room: number): string {\n  return \"\";\n",
    ["no category page names a vendor whose terms nothing on record contradicts"]],
];
